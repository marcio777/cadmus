...
3. You can see from line 14, that in Java integers can both be positive and negative.
4. In Exercise 1, we told you that `System.out.println()` takes a string as its only argument. Actually there are several different versions of `System.out.println()`, including one which prints out strings and another that prints out integers.
5. It is important you recognise that while `System.out.println(3229 - 234234)` makes sense `System.out.println(3229) - 234234` does not. When Java sees `System.out.println(3229 - 234234)` it **evaluates** its arguments and then passes the evaluated arguments to `System.out.println()`. In other words it simplifies its arguments, so `3229 - 234234` becomes `-231005` which then is **passed** to `System.out.println`. With `System.out.println(3229) - 234234` however, this makes no sense as you're saying print `3229` and subtract 234234 from whatever `System.out.println(3229)` equals. However `System.out.println(3229)` is a method and doesn't equal a value or rather **return** any value.
6. In line 16 you see you can add, or rather **concatenate** a string and an integer by using the `+` **operator**. For example, `"aaa" + 5` becomes `"aaa5"`.
7. In line 17, in the expression `"How tall am I in cm?" + (2*100)`, `(2 * 100)` is first simplified to `200` and then concatenated with `"How tall am I in cm? "`. Although technically you don't need to put parentheses, as `*` has a higher precedence in BIDMAS than `+`.
